...
_ical_service_config: Optional[str] = (
    ...
    if _ical_service is None or _ical_service_config != current_config:
        ...
        if not feed_configs:
            ...
            try:
                ...
                if configs_str.strip().startswith(
                    "["
                ) or configs_str.strip().startswith("{"):
                ...
                if '"url"' not in configs_str and '"name"' not in configs_str:
                    ...
                    if feed_configs:
                        logger.info(
                            f"Parsed {len(feed_configs)} feeds from delimited config"
                        )
                else:
                    logger.error(
                        "Config appears to be malformed JSON, not attempting delimiter split"
                    )
                    return None
        ...
        try:
            ...
            logger.info(
                f"Initialized iCalendar service with {len(feed_configs)} feeds"
...
def get_current_datetime_tool() -> Dict[str, Any]:
    """MCP tool wrapper for get_current_datetime."""
...
def get_server_status() -> Dict[str, Any]:
    ...
    status = {"server": "CalendarMCP", "version": "1.0.0", "services": {}}
...
if __name__ == "__main__":
    ...
    if not os.getenv("ICAL_FEED_CONFIGS"):
        logger.warning(
            "No iCalendar feeds configured. Set ICAL_FEED_CONFIGS in .env.local or .env"
        )
